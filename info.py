...
CHANNELS = [int(ch) if id_pattern.search(ch) else ch for ch in environ.get('CHANNELS', '-1003337417077 -1003703513064 -1003965891384').split()]  # Channel id for auto indexing (make sure bot is admin)
...
MULTI_FSUB = [int(channel_id) for channel_id in environ.get('MULTI_FSUB', '-1003708952638 -1002784619700 -1003779766425').split() if re.match(r'^-?\d+$', channel_id)]  # Channel for force sub (make sure bot is admin)
